...
export async function fetchPostDetailsFromBrowser(page: Page, storage: Deno.Kv, idList: Array<string>) {
    for (const id of idList) {
        ...
        for (let i = 0; i < 5; i++) {
            try {
                ...
                const result: any = page.evaluate(`(async () => {${fetchPostDetails.toString().replace('{{id}}', id)};return await fetchPostDetails();})()`)
                if (result.data) {
